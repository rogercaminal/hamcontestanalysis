...
class TableBase(ABC):
    ...
    This abstract class serves as a base interface for the different tables,
    ...
    def columns(self):
        ...
        if not all([k in ("name", "id", "type") for k in self._columns[0].keys()]):
            raise ValueError("The keys of each dictionary must be 'name', 'id', and 'type'")
        if not all([d["type"] in ("numeric", "text", "datetime") for d in self._columns]):
            raise ValueError("The values of type must be 'numeric', 'text' or 'datetime'.")
    ...
    def show(self, page_size: int = 250) -> None | DataTable:
        """Create table.
        Args:
            page_size (int): number of rows to display. Defaults to 250.

        Returns:
            None | DataTable: DataTable containing the table
        """
