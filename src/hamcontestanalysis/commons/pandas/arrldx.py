...
def compute_contest_score(data: DataFrame) -> DataFrame:
    ...
    """
    logger.info("Process ARRL DX contest")
    data = (
        data.join(
        ...
        .join(
            data.drop_duplicates(subset=["band", "exchange"], keep="first")[
                "datetime"
            ].rename("exchange_first_occurrence")
        )
        .assign(
            ...
            potential_qso_points=3,
            qso_points=lambda x: x["is_valid"] * x["potential_qso_points"],
            is_mult=lambda x: np.where(
                x["mycountry"].isin(["United States", "Canada"]),
                (x["datetime"] == x["dxcc_first_occurrence"]).astype(int),
                (x["datetime"] == x["exchange_first_occurrence"]).astype(int)
            ),
