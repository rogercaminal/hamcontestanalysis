...
def input_data():
    return read_parquet(
        "tests/resources/plots/common/plot_base__get_inputs__ef6t_2022_cr6k_2022.parquet"
    )
...
def validation_data():
    return read_parquet(
        "tests/resources/plots/common/plot_log_heatmap__prepare_dataframe__ef6t_2022_cr6k_2022.parquet"
    )


def test_class_arguments():
    plot = PlotLogHeatmap(
        contest="cqww", mode="cw", callsigns_years=[("EF6T", 2022), ("CR6K", 2022)]
    )
...
def test_prepare_dataframe(mocker, input_data, validation_data):
    mocker_get_inputs = mocker.patch(
        "hamcontestanalysis.plots.plot_base.PlotBase._get_inputs",
