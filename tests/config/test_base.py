...
def test_base_settings_subclass_attributes():
...
def test_base_settings_immutable():
    settings = MockTestSettings(simple_setting="bar", sub_setting={"key_1": 1})
    with pytest.raises(TypeError):
        settings.simple_setting = "bar2"


def test_base_settings_no_extra_fields():
    with pytest.raises(ValueError):
        MockTestSettings(simple_setting="bar", sub_setting={"key_1": 1}, bar="baz")


def test_base_settings_override_from_env_var():
    with patch.dict(
        os.environ,
        {
